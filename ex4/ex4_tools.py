...
def decision_boundaries(classifier, X, y, num_classifiers=1, weights=None, fig=None):
    """
    ...
    plt_obj = plt if fig is None else fig
    plt_obj.pcolormesh(xx, yy, Z, cmap=cm)
    # Plot also the training points
    if weights is not None:
        plt_obj.scatter(X[:, 0], X[:, 1], c=y, s=weights, cmap=cm_bright)
    else:
        plt_obj.scatter(X[:, 0], X[:, 1], c=y, cmap=cm_bright)
    ...
    if fig is None:
        ...
        plt_obj.title.set_text(f'num classifiers = {num_classifiers}')
    plt.draw()
